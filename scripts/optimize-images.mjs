...
const dimensionsFilePath = path.join(
  projectRoot,
  "src",
  "data",
...
async function optimizeAndExtractDimensions() {
  ...
  try {
    ...
    for (const mdPath of markdownFiles) {
      ...
      try {
        ...
        if (!imagePathRelativeMd) {
          console.warn(
            `No image path found in frontmatter for ${mdFilename}, skipping dimension extraction.`,
          );
          continue;
        }
        ...
        if (
          pathParts.length < 4 ||
          pathParts[pathParts.length - 3] !== "images"
        ) {
          console.warn(
            `Unexpected image path structure in ${mdFilename}: ${imagePathRelativeMd}, skipping dimension extraction.`,
          );
          continue;
        }
        ...
        if (!originalWidth || !originalHeight) {
          console.warn(
            `Could not get dimensions for image ${relativeImagePathKey} referenced in ${mdFilename}, skipping.`,
          );
          continue;
        }
        ...
        if (err.code === "ENOENT") {
          console.error(
            `Error processing ${mdFilename}: Source image not found at ${err.path}`,
          );
        } else {
          console.error(
    ...
    const imageFilesToOptimize = await glob(
    ...
    console.log(
      `Found ${imageFilesToOptimize.length} images in source directory to optimize.`,
    );
    ...
    for (const imagePath of imageFilesToOptimize) {
      ...
      try {
        ...
        if (!originalWidth) {
          console.warn(
            `Could not get width for ${relativePath} during optimization, skipping.`,
          );
          continue;
        }
    ...
    console.log(
      `Successfully optimized images processed: ${processedImageCount}`,
    );
